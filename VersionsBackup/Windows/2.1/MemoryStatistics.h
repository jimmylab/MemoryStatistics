 ...
 * the maximum memory usage in its lifetime, by creating
 * a specially optimized monitoring thread.
...
public:
	 ...
	 * @param NoErrors (Optional)
	 *         Ignore Errors during the statisting process.
	 */
	MemoryStats( int PID, bool instant_start = true, int Interval = DefaultCheckInterval, bool NoErrors = true, bool Debug=true )
		: TargetPID(PID), CheckInterval(Interval),
		//p_WorkingSet(pmc.WorkingSetSize), p_PeakWs(pmc.PeakWorkingSetSize),
		IgnoreErrors(NoErrors) {
		...
		#ifndef PRODUCTIONAL_EDIT
		std::cerr<<"Message: Started counting with ";
		if ( !Interval ) std::cerr<<"no";
		else std::cerr<<Interval<<" ms of";
		std::cerr<<" delay.\n";
		#endif
	...
	SIZE_T GetMax( bool StopChecking = true ) {
		...
		#ifndef PRODUCTIONAL_EDIT
		std::cerr<<"Result: Max Memory: "<<MaxCounter<<'\n';
		#endif
	...
	~MemoryStats()
	{
		...
		#ifndef PRODUCTIONAL_EDIT
		std::cerr<<"Message: Ended Stats. Max Memory: "<<MaxCounter<<'\n';
		#endif
	}
...
DWORD WINAPI MemoryCheckThread( LPVOID arg )
{
	...
	if ( RecordBroken ) {
		#ifndef PRODUCTIONAL_EDIT
		std::cerr<<"Message: Triggered RecordBroken Event.\n";
		#endif
